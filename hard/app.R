...
explorapp <-function(ui,server){
  ...
  ui <- shiny::fluidPage(shiny::mainPanel(
    shiny::tabsetPanel(
      ...
      shiny::tabPanel(
        ...
        shiny::fluidRow(
          shiny::column(12,
                        shiny::h4("Draw your own data by clicking on the plot"),
                        shiny::p("Select a class, then click on the plot to add points. Click 'Fit Models' to see boundaries.")
          )
        ),
        shiny::fluidRow(
          shiny::column(3,
                        shiny::selectInput("draw_class", "Select Class",
                                           choices = c("sim1", "sim2", "sim3"), selected = "sim1"),
                        shiny::actionButton("fit_draw", "Fit Models", class = "btn-primary"),
                        shiny::br(), shiny::br(),
                        shiny::actionButton("clear_draw", "Clear All Points", class = "btn-danger"),
                        shiny::br(), shiny::br(),
                        shiny::verbatimTextOutput("draw_info")
          ),
          shiny::column(9,
                        shiny::plotOutput("draw_plot", click = "draw_click",
                                          width = "500px", height = "500px")
          )
        ),
        shiny::fluidRow(
          shiny::plotOutput("draw_boundaries", height = "400px")
        )
      )
    )
  ))
  ...
  server <- function(input, output) {
    ...
    # Reactive storage for drawn points
    drawn_points <- shiny::reactiveVal(data.frame(X1 = numeric(0), X2 = numeric(0), Sim = character(0)))
    ...
    shiny::observeEvent(input$draw_click, {
      ...
      updated <- rbind(drawn_points(), new_point)
      drawn_points(updated)
    })
    
    # Clear all points
    shiny::observeEvent(input$clear_draw, {
      drawn_points(data.frame(X1 = numeric(0), X2 = numeric(0), Sim = character(0)))
    })
    
    # Show info about drawn points
    output$draw_info <- shiny::renderText({
      pts <- drawn_points()
      if (nrow(pts) == 0) return("No points drawn yet.\nClick on the plot to add points.")
      tbl <- table(pts$Sim)
      paste(c("Points drawn:", paste(names(tbl), tbl, sep = ": ")), collapse = "\n")
    })
    
    # Draw the canvas with current points
    output$draw_plot <- shiny::renderPlot({
      pts <- drawn_points()
      colors <- c("sim1" = "#E41A1C", "sim2" = "#377EB8", "sim3" = "#4DAF4A")
      
      plot(0, 0, type = "n", xlim = c(-5, 5), ylim = c(-5, 5),
           xlab = "X1", ylab = "X2",
           main = "Click to draw points")
      grid()
      
      if (nrow(pts) > 0) {
        point_cols <- colors[pts$Sim]
        points(pts$X1, pts$X2, col = point_cols, pch = 19, cex = 1.5)
        legend("topright", legend = names(colors), col = colors, pch = 19, title = "Class")
      }
    })
    
    # Fit models and show boundaries on button click
    output$draw_boundaries <- shiny::renderPlot({
      shiny::req(input$fit_draw)
      
      pts <- shiny::isolate(drawn_points())
      
      if (nrow(pts) < 10) {
        plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
        text(0, 0, "Please draw at least 10 points\nacross at least 2 classes", cex = 1.5)
        return()
      }
      
      classes <- unique(pts$Sim)
      if (length(classes) < 2) {
        plot(0, 0, type = "n", axes = FALSE, xlab = "", ylab = "")
        text(0, 0, "Please draw points for at least 2 classes", cex = 1.5)
        return()
      }
      
      pts$Sim <- as.factor(pts$Sim)
      
      grilla <- base::expand.grid(
      ...
      # Rpart boundary
      pl_rpart <- tryCatch({
        mod <- rpart::rpart(Sim ~ X1 + X2, data = pts)
        grilla$pred <- predict(mod, newdata = grilla, type = "class")
        ggplot2::ggplot(grilla) +
          ggplot2::geom_point(ggplot2::aes(x = X1, y = X2, color = pred), alpha = 0.2) +
          ggplot2::geom_point(data = pts, ggplot2::aes(x = X1, y = X2, color = Sim), size = 3) +
          ggplot2::scale_colour_brewer(type = "qual", palette = "Dark2") +
          ggplot2::theme_bw() +
          ggplot2::theme(legend.position = "none", aspect.ratio = 1) +
          ggplot2::labs(title = "Rpart")
      }, error = function(e) {
        ggplot2::ggplot() + ggplot2::annotate("text", x = 0, y = 0, label = paste("Rpart error:", e$message)) + ggplot2::theme_void()
      })
      
      # RandomForest boundary
      pl_rf <- tryCatch({
        mod <- randomForest::randomForest(Sim ~ X1 + X2, data = pts)
        grilla$pred <- predict(mod, newdata = grilla, type = "class")
        ggplot2::ggplot(grilla) +
          ggplot2::geom_point(ggplot2::aes(x = X1, y = X2, color = pred), alpha = 0.2) +
          ggplot2::geom_point(data = pts, ggplot2::aes(x = X1, y = X2, color = Sim), size = 3) +
          ...
          ggplot2::theme_bw() +
          ggplot2::theme(legend.position = "none", aspect.ratio = 1) +
          ggplot2::labs(title = "RandomForest")
      }, error = function(e) {
        ggplot2::ggplot() + ggplot2::annotate("text", x = 0, y = 0, label = paste("RF error:", e$message)) + ggplot2::theme_void()
      })
      
      # PPtreeExt boundary
      pl_pptree <- tryCatch({
        mod <- PPtreeExt::PPtreeExtclass(Sim ~ X1 + X2, data = pts, PPmethod = "LDA")
        grilla$pred <- as.factor(predict(mod, newdata = grilla[, c("X1","X2")])[[2]])
        ggplot2::ggplot(grilla) +
          ggplot2::geom_point(ggplot2::aes(x = X1, y = X2, color = pred), alpha = 0.2) +
          ggplot2::geom_point(data = pts, ggplot2::aes(x = X1, y = X2, color = Sim), size = 3) +
          ggplot2::scale_colour_brewer(type = "qual", palette = "Dark2") +
          ggplot2::theme_bw() +
          ggplot2::theme(legend.position = "none", aspect.ratio = 1) +
          ggplot2::labs(title = "PPtreeExt")
      }, error = function(e) {
        ggplot2::ggplot() + ggplot2::annotate("text", x = 0, y = 0, label = paste("PPtreeExt error:", e$message)) + ggplot2::theme_void()
      })
      
      gridExtra::grid.arrange(pl_rpart, pl_rf, pl_pptree, ncol = 3)
    })
